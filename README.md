...
To properly explain this analysis, it's essential to look at it step-by-step. We'll go over the analysis section by section. This will allow us to understand each outcome better. But before we do that, it's important to import dependencies, read all the files provided for this analysis, and merge them.
...
The first part of this analysis is an essential one. This section calculates each city type’s average fare per ride and driver. This information will show us the different fares between the city types. We first need to understand how many total rides were completed in each city type, how many drivers each city type has, and what are the fares for each city type by using the **groupby()** and **count()** functions:
...
Even though urban cities completed a lot more rides than suburban and rural cities, they have a considerably higher number of drivers. Thus the average fares are higher in rural and suburban areas. Using the ***total fares per city type / total rides per city type***, we can see the average fare per ride in all areas. In addition, each driver earns more in rural cities than in suburban and urban cities. The average fare per driver is calculated by ***total fares per city type / total number of drivers per city type***. The results are as follows:
...
The second part of the analysis includes finding the total weekly fares by city type and plotting it. This part requires looking at the sum of fares based not only on type but on a date as well. To find this, we need to group our original merged data by ***date*** and ***type*** and compute the sum of ***fares***. 
...
* Average fare per ride is very high in rural areas ($34.62), less in suburban($30.97) and urban($24.52) cities.
* Average fare per driver is different accordingly: $55.48, $39.50, $16.57
* Despite average fares being higher in rural and suburban areas, urban cities have the highest profit over time based on the ride number.
...
**2.** The fares are high in rural and suburban cities. Even though this can depend on the lack of drivers and demand, there is a need for extensive research to understand whether or not the high fares could cause a lack of demand and, thus, lack of drivers. This matter is intertwined, and research is required, as mentioned above.

**3.** In this analysis, we're looking at a couple of months in 2019. This can damage our analysis. The ride_share data is vast, and only looking at this short timeframe could jeopardize the analysis. Additional analysis could be conducted using more extended periods than January to April 2019.
